...
<script>
    function changeTaskStatus(id,status){
        $.ajax({
            url:"${pageContext.request.contextPath}/office/office/updateTaskStatus",
            type:"post",
...
<tr bgcolor="#E7E7E7">
	<td height="24" colspan="12" background="${pageContext.request.contextPath}/resources/skin/images/tbg.gif">&nbsp;我的任务信息&nbsp;</td>
</tr>
<tr align="center" bgcolor="#FAFAF1" height="22">
    ...
    <c:forEach var="task" items="${taskList}">
        <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
            ...
            <td align="center">${activeUser.ename}</td>
            <td align="center">
                <c:if test="${task.status==1}">
                    <span style="color:red" id="status${task.id}">未开始</span>
                </c:if>
                <c:if test="${task.status==2}">
                    <span style="color:blue" id="status${task.id}">进行中...</span>
                </c:if>
                <c:if test="${task.status==3}">
                    <span style="color:green" id="status${task.id}">已完成</span>
                </c:if>
            </td>
            ...
            <td><a href="javascript:changeTaskStatus(${task.id},2);">开始任务</a> | <a href="javascript:changeTaskStatus(${task.id},3);">任务完成</a> | <a href="task-look.html">查看详情</a></td>
        </tr>
    </c:forEach>
